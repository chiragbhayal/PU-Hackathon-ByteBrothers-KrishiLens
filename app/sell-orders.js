...
export default function SellOrdersScreen() {
  ...
  const sellOrders = [
    {
      id: 1,
      product: language === 'hi' ? 'टमाटर' : 'Tomato',
      quantity: '200 kg',
      price: '₹40/kg',
    ...
    {
      id: 2,
      product: language === 'hi' ? 'प्याज' : 'Onion',
      quantity: '150 kg',
      price: '₹25/kg',
        ...
        <ScrollView style={styles.scrollContent}>
          {activeTab === 'create' ? (
            <View style={styles.createForm}>
              ...
              <View style={styles.inputGroup}>
                <Text style={styles.label}>{language === 'hi' ? 'विवरण' : 'Description'}</Text>
                <TextInput 
                  style={[styles.input, { height: 80 }]} 
                  placeholder={language === 'hi' ? 'उत्पाद का विवरण' : 'Product description'} 
                  multiline 
                />
              </View>
              ...
              {sellOrders.map((order) => (
                <View key={order.id} style={styles.orderCard}>
                  <View style={styles.orderHeader}>
                    ...
                    <View style={[styles.statusBadge, { backgroundColor: order.status === 'active' ? '#FF5722' : '#4CAF50' }]}>
                      <Text style={styles.statusText}>
                        {order.status === 'active' ? (language === 'hi' ? 'बिक्री के लिए' : 'For Sale') : (language === 'hi' ? 'बेचा गया' : 'Sold')}
                      </Text>
                    </View>
                  </View>
                  ...
                  <View style={styles.orderActions}>
                    <TouchableOpacity style={styles.editButton}>
                      ...
                      <Text style={styles.actionText}>{language === 'hi' ? 'संपादित करें' : 'Edit'}</Text>
                    </TouchableOpacity>
                    <TouchableOpacity style={styles.deleteButton}>
                      <Ionicons name="trash-outline" size={16} color="#F44336" />
                      <Text style={styles.actionText}>{language === 'hi' ? 'हटाएं' : 'Delete'}</Text>
                    </TouchableOpacity>
                  </View>
                </View>
              ))}
...
const styles = StyleSheet.create({
  ...
  submitButton: { backgroundColor: '#FF5722', padding: 16, borderRadius: 8, alignItems: 'center', marginTop: 20 },
  ...
  orderActions: { flexDirection: 'row', marginTop: 12, gap: 16 },
  editButton: { flexDirection: 'row', alignItems: 'center' },
  deleteButton: { flexDirection: 'row', alignItems: 'center' },
  actionText: { fontSize: 14, marginLeft: 4, color: '#666' },
});